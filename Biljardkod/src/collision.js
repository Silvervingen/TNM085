...
// inP is the current position, prevPos is the position from last iteration, inV is the current velocity, n is the slot in the spheres array
// outV is the velocity after edge collision, or the same, outP is the position after edge collision
// @TODO Might be possible to do a void function since vectors are objects and passed by reference, would be faster
function edgeDetection(inP, prevPos, inV, n) {
    ...
    if (inP.distanceTo(new THREE.Vector2(0, -2 * radius)) <= 2.9 * radius || (inP.distanceTo(new THREE.Vector2(width + 2 * radius, -2 * radius)) <= 2.9 * radius ||
        ...
        if (n == 0) {
            ...
            // @TODO right now it just teleports to starting point, make something better
...
// v1 and v2 are current velocity of the balls, p1 and p2 are current positions of the balls, prevPos1-2 are positions of the two balls from last iteration, n and k are the slots in the spheres array
// outV1 and outV2 are the velocity after collision, outP1 and outP2 are the positions after collision
// @TODO Vectors are objects and passed by reference, could make a void function and avoid copying everything, would be faster
// @TODO Messy code, could be writter better
